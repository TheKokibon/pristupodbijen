...
const About = () => {
  ...
  // State to track the audio play status
  const [isPlayingAudio1, setIsPlayingAudio1] = useState(false);

  // Function to toggle the play/pause of audio 1
  const togglePlayAudio1 = () => {
    if (isPlayingAudio1) {
      audioRef1.current.pause(); // Pause audio if playing
      setIsPlayingAudio1(false);
    } else {
      audioRef1.current.play(); // Play audio if paused
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-grow flex flex-col justify-center">
        <div className="flex flex-col justify-center items-center">
          ...
          >
            {isPlayingAudio1 ? "⏸ Pauziraj" : "🔊 Pročitaj"}
          </button>
          <audio
            ref={audioRef1}
            src={Onama}
        ...
        <div className="w-full md:w-2/3 lg:w-1/2 mx-auto flex flex-col justify-center p-4 md:p-6 lg:p-8">
          <p className="font-taviraj italic text-base md:text-lg lg:text-xl text-center mb-4">
            Mi smo Pokret za inkluzivno studiranje, sačinjen od studenata
            psihologije na Filozofskom fakultetu u Beogradu.
          </p>

          <p className="font-heebo text-sm md:text-base lg:text-lg text-justify leading-relaxed md:leading-relaxed lg:leading-loose">
            Formirani smo u novembru 2023. godine s ciljem da podignemo svest o
            izazovima sa kojima se suočavaju studenti sa invaliditetom i da se
            borimo za pristupačnost fakulteta koji pohađamo, kao i univerziteta.
            Ovo je od istinske važnosti jer omogućava svim studentima, bez
            obzira na njihove fizičke ili mentalne sposobnosti, ravnopravan
            pristup obrazovanju i punom uživanju akademskog života.
            Pristupačnost je ključna za stvaranje inkluzivnog okruženja u kojem
            svi imaju jednake prilike za uspeh i razvoj, bez diskriminacije i
            prepreka.
          </p>
        </div>
